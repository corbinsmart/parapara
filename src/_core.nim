...
const
  ...
  fish_images = [
    staticRead("assets/goldfish.png")
  ]
  gravity = 100
  deceleration = 0.9
  damping = 0.5
  maxVelocity = 300f
  maxJumpVelocity = 1000f
  maxFishJumpVelocity = 2000f
  animationSecs = 0.2

var
  imageEntities: array[3, ImageEntity]
  fishEntities: array[10, ImageEntity]
  ...
  staticRuleset(Fact, FactMatch):
    ...
    rule doFishJump(Fact):
      ...
      cond:
        ...
        abs(fy-py) < 50
      then:
        session.insert(Player, YVelocity, -1 * maxFishJumpVelocity)
    rule doFishCursorJump(Fact):
      what:
        (FishCursor, X, fx)
        (FishCursor, Y, fy)
        (Player, X, px)
      ...
      then:
        session.insert(Player, YVelocity, -1 * maxFishJumpVelocity)
    # mouse
    rule doMouseClick(Fact):
      what:
        (Global, MouseClick, uh)
        (Global, MouseX, x, then=false)
        (Global, MouseY, y, then=false)
      then:
        session.insert(nextId, X, x)
        session.insert(nextId, Y, y)
        session.insert(nextId, EntType, "fish")
        echo "INSERT FISH ", nextId, " @ ", x, ", ", y
        nextId += 1
    rule doMouseMove(Fact):
      what:
        (Global, MouseX, x)
        (Global, MouseY, y)
      then:
        echo "MOUSE @ ", x, ", ", y
        session.insert(FishCursor, X, x)
        session.insert(FishCursor, Y, y)
...
proc init*(game: var Game) =
  ...
  var fish_width, fish_height, fish_channels: int
  let fish_data = stbi.loadFromMemory(cast[seq[uint8]](fish_images[0]), fish_width, fish_height, fish_channels, stbi.RGBA)
  let uncompiled_fish_image = initImageEntity(fish_data, fish_width, fish_height)
  for i in 0 ..< fishEntities.len:
    fishEntities[i] = compile(game, uncompiled_fish_image)
  ...
  # init fish
  session.insert(FishCursor, X, 0f)
  session.insert(FishCursor, Y, 0f)
...
proc tick*(game: Game) =
  ...
  # fishes
  let fishIndexes = session.findAll(rules.getFish)
  echo "fishindexes ", fishIndexes
  for index in fishIndexes:
    let i = index
    closureScope:
      var fish_image = fishEntities[i+1]
      let fish = session.get(rules.getFish, i)
      fish_image.project(float(worldWidth), float(worldHeight))
      fish_image.translate(fish.x, fish.y)
      fish_image.scale(60,40)
      render(game, fish_image)

  # fish cursor
  var fish_image = fishEntities[0]
  let fish = session.query(rules.getFishCursor)
  fish_image.project(float(worldWidth), float(worldHeight))
  fish_image.translate(fish.x, fish.y)
  fish_image.scale(60,40)
  render(game, fish_image)
